...
export function CreateSessionForm({
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (!dailyCode)
        throw new Error('No daily code found for this date. Create one first.')
      if (formData.cohortIds.length === 0)
        throw new Error('Select at least one cohort')
  ...
  const getExistingSessionsText = () => {
    ...
    return `(${uniqueCombinations.size} existing session${
      uniqueCombinations.size > 1 ? 's' : ''
    })`
  }
      ...
      <VStack spacing={6} align="stretch">
        ...
        {dailyCode ? (
          <Alert status="success" borderRadius="md">
            <AlertIcon />
            <Text fontSize="sm">
              Daily code <strong>{dailyCode}</strong> is active for{' '}
              {formData.date}
            </Text>
          </Alert>
        ) : (
          <Alert status="warning" borderRadius="md">
            <AlertIcon />
            <Text fontSize="sm">
              No daily code found for {formData.date}. Create a daily code
              first.
            </Text>
          </Alert>
        )}

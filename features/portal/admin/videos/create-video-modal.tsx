...
import { createVideo } from '../../../../lib/utils/video.utils'
...
const validationSchema = Yup.object().shape({
  ...
  videoImage: Yup.string().optional(), // Make optional during form validation
  author: Yup.string()
...
export const CreateVideoModal: React.FC<CreateVideoModalProps> = ({
  ...
  const handleSubmit = async (values: VideoFormValues) => {
    ...
    if (!selectedImageFile && !values.videoImage) {
      setImageError('Please select a video image')
      return
    }
    ...
    try {
      ...
      toast({
        title: 'Success! 🎉',
        description: 'Video created successfully with access code',
        status: 'success',
        duration: 5000,
      })
              ...
              <ModalBody>
                <VStack spacing={4}>
                  <Alert status="info" borderRadius="md">
                    <AlertIcon />A unique access code will be automatically

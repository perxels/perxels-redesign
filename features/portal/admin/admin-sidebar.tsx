...
export const AdminSidebar = () => {
  ...
  const handleKeyPress = useCallback((event: React.KeyboardEvent, href: string, label: string) => {
      ...
      {sidebarItems.map((item) => {
        ...
        return (
          <Box
            ...
            onClick={() => !isLoading && handleNavigation(item.href, item.label)}
          ...
          >
            <Box position="relative">
              ...
              {item.label === 'Notifications' && unreadCount > 0 && !isLoading && (
                <Badge
                  position="absolute"
                  top="-8px"
                  right="-8px"

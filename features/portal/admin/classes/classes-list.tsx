...
export const ClassesList = () => {
  ...
  const calculateEndDateFromSyllabus = useCallback((startDate: Date, syllabus: Syllabus): Date => {
  ...
  const fetchAllStudentCounts = useCallback(async (cohortNames: string[]): Promise<Record<string, number>> => {
    try {
      ...
      querySnapshot.docs.forEach(doc => {
        ...
        if (cohort && cohortNames.map(c => c.toUpperCase()).includes(cohort)) {
  ...
  const fetchSyllabusData = useCallback(async (classes: ClassData[]) => {
    ...
    try {
      ...
      const syllabusIds = Array.from(new Set(classes.map(c => c.syllabusId).filter(Boolean)))
      ...
      const classesWithSyllabusData: ClassWithSyllabus[] = classes.map((klass) => {
        ...
        classWithSyllabus.actualStudentCount = studentCounts[cohortUpper] || 0
